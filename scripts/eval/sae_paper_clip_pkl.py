...
@torch.no_grad()
def _sims_images_to_texts(images: List[Image.Image], txt_feats: torch.Tensor) -> torch.Tensor:
    if len(images) == 0 or txt_feats.numel() == 0:
        return torch.empty(0)
    img_feats = _encode_images(images)
    sims = []
    for tfeat in txt_feats:
        tfeat_exp = tfeat.unsqueeze(0).expand(img_feats.size(0), -1)
        sim_vals = cos(img_feats, tfeat_exp)
        sims.append(sim_vals)
    sims = torch.stack(sims, dim=1)
    sims = (sims + 1.0) / 2.0  # normalized to [0,1]
    return sims
...
def process_file(file_path: Path, ref_texts: List[str]) -> List[Dict]:
    ...
    for general_prompt, layer_name, specific_prompt, images in tqdm(records, desc=f"Processing {file_path.name}", leave=False):
        ...
        sims_general = _sims_images_to_texts(pil_images, txt_cache[gp_key])
        sim_vs_general = float(sims_general.mean().item()) if sims_general.numel() else None
        if specific_prompt:
            sp_key = _norm_text(specific_prompt)
            if sp_key not in txt_cache:
                txt_cache[sp_key] = _encode_texts([specific_prompt])
            sims_specific = _sims_images_to_texts(pil_images, txt_cache[sp_key])
            sim_vs_specific = float(sims_specific.mean().item()) if sims_specific.numel() else None
        else:
            sim_vs_specific = None
        row = {
            ...
            "sim_vs_general": None if sim_vs_general is None else round(sim_vs_general, 4),
            "sim_vs_specific": None if sim_vs_specific is None else round(sim_vs_specific, 4),
        }
        for ref in ref_texts:
            row[f"sim::{ref}"] = None
        if ref_texts:
            sims_refs = _sims_images_to_texts(pil_images, refs_feats)
            means = sims_refs.mean(dim=0).tolist()
            for ref, val in zip(ref_texts, means):
                row[f"sim::{ref}"] = round(float(val), 4)
        out.append(row)
    return out
